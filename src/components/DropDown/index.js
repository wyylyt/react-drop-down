...
class DropDown extends Component {
  ...
  checkListItem=(val)=>{
    ...
    this.setState({
      selectedVal:val,
      dropMenuShow:false,
      selectedValMatch:selectedValMatch
    })
  ...
  inputChange=(event)=>{
    ...
    const selectedValMatch = this.matchInputval(val);
    const bool = val.length > 0 ? false : true;
  ...
  matchInputval=(val)=>{
    ...
    const arr = listData.filter((ele)=>{
       return ele === val
    })
    const bool = arr.length > 0 ? true : false;
    return bool;
  }
  ...
  filterData=(val)=>{
    const { listData } = this.state;
    const data = listData.filter((ele)=>{
      return ele.indexOf(val) !== -1;
    })
    return data;
  }
  ...
  render() {
    const { selectedVal, renderData, dropMenuShow, selectedValMatch, ifHideDelIcon } = this.state;
    const {placeholder,ifLoading} = this.props;
    return (
      <div className="container">
          <input className={selectedValMatch === true ? "drop-input" : "drop-input drop-input-notmatch"} 
            placeholder={placeholder}
            ref={(input) => this.textInput = input}
            onChange={this.inputChange} 
            onClick={this.inputClick}
            value={selectedVal}
            disabled={ifLoading ? true : false}
            />
          ...
          <span className="drop-arrow" onClick={this.dropArrowClick}>
            <i className={ifLoading ? "loading-state" : "caret"}></i>
          </span>
          <div onClick={(e)=> {this.stopPropagation(e)}}
